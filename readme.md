...
房主的功能：
- 新建房间
- 解散房间（随时），其他房间的玩家强制返回大厅
- 在房间切换游戏（可以换游戏类型及PK模式，如果房客没有离开，会强制跟随跳转至新游戏）
- 重新开始对局（当所有房客处于“已准备”状态时才可点击）
- 踢人（将未准备或不活跃的房客踢出房间）
- 离开房间（离开后若房间无人，则自动立刻销毁房间）
- 加入房间（加入别人的房间后自动成为房客）

房客的功能：
- 加入房间（跟随房主同步切换游戏类型，除非主动离开）
- 离开房间
- 准备（点击后头上显示 ✅，房主和所有玩家可见；准备后不可切换游戏，需等房主点击开始）
- 取消准备（点击后取消 ✅，然后房主不可点击开始）

房间：
- 是一个极其重要的独立实体
- 生命周期管理极其干脆：人数为 0 时【自动且立刻解散】，拒绝产生滞留的“幽灵房间”
- 每个房间有自己的游戏类型、PK模式和游戏状态
- 房间内有完整的玩家信息、准备状态，以及底层游戏引擎实例
- 房间状态
  - 等待加入 (Waiting - 显示等待室 UI，房客准备，房主开始)
  - 游戏进行中 (Playing)
  - 游戏结束 (Finished - 显示结算 UI，房主可重新开始， )
- 房间解散或踢人会产生 WebSocket `room_dismissed` / `kicked` 事件通知，使客户端清理缓存并强制退出

建议补充的核心功能
房间加入门槛（私密房间/密码）： 现在任何人在大厅都能看到房间列表并一键加入。如果几位朋友想私下玩，很容易被大厅里的路人抢占位置。建议给新建房间加上一个简单的 isPrivate 或 4位数字密码选项。
房客“踢房主”机制（房主不活跃踢出）： 如果房主在房间里挂机（发呆 5 分钟没有任何操作），此时其他人已经准备了，但房主就是不点开始。这个时候由于别人不是房主，无法踢人。可以增加一个机制：如果所有房客准备超过 N 分钟房主未操作，系统自动将房主降级或踢出，把房主转移给别人。
“正在游戏中”的观战/断线重连： 说明里提到了 Waiting，Playing，Finished。目前如果房间处于 Playing，其他玩家不小心点进来，系统是直接拒绝还是允许他以“观战者（Spectator）”身份进入？由于扫雷等游戏完全在前端计算，中途加入很难同步整个棋盘状态。必须在后端明确规定：当 Room state == Playing 时，拒绝任何非原玩家的加入请求。
