...
以后我增加什么新功能，你按需要同时修改里面的内容，相当于项目的说明书， 同时还要考虑多语言，多主题的支持啊， 还要考虑可扩展性，比如以后我

遵守四大准则：

- 项目说明书同步更新：每完成一个新功能，我都会自动去修改 docs/FEATURES.md 和 docs/CHANGELOG.md，保证文档永远和代码同步。
- 多语言与多主题（I18n & Theming）：所有新增的 UI 文本我都会抽离到 i18n.service.ts 的翻译字典里，绝不硬编码；所有的颜色样式我都严格按照 CSS 变量来写，保证 Light/Dark 主题无缝切换。
- 高可扩展性设计：后端接口和前端模块必须松耦合。像目前的扫雷一样，以后加任何新游戏，只需要实现 GameEngine 接口并新建一个独立路由模块即可，绝不牵一发而动全身。
- 要能适配多种设备：手机、平板、电脑等
