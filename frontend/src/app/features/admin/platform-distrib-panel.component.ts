    ...
    @if (loading()) {
      ...
      <div class="flex flex-wrap gap-3">
        @for (p of platforms; track p.id) {
          <div class="relative group/tip flex items-center gap-2 bg-[var(--color-bg-main)]/60 rounded-xl px-3 py-2 border border-[var(--color-border-card)]/40 hover:border-cyan-500/30 transition-colors">
            <!-- Tooltip -->
            <div class="pointer-events-none absolute bottom-full left-0 mb-2 w-56 z-20
...
export class PlatformDistribPanelComponent {
  distribKey = input('');
  loading    = input(false);
  copy       = output<{ platformId: PlatformId; lang: 'en' | 'zh'; url: string }>();
  ...
  emit(platformId: PlatformId, lang: 'en' | 'zh', url: string) {
    this.copy.emit({ platformId, lang, url });
  }
}
