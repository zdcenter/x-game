  ...
  template: `
    <div class="flex-grow flex flex-col items-center p-8 transition-colors duration-300">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl w-full">
        ...
        <!-- Minesweeper Card -->
        <a routerLink="/games/minesweeper" class="group relative overflow-hidden rounded-3xl border transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)] cursor-pointer"
          ...
          <!-- Card Content -->
          <div class="p-6">
            <h2 class="text-2xl font-bold mb-2">{{ i18n.t('app.title')() }}</h2>
            <p class="opacity-70 text-sm">
              {{ i18n.t('lobby.minesweeper.desc')() }}
            </p>
            <div class="mt-4 flex flex-wrap gap-2">
              <span class="px-2 py-1 text-xs font-semibold rounded bg-black/20 border border-slate-600/50 text-slate-300">Single Player</span>
              <span class="px-2 py-1 text-xs font-semibold rounded bg-black/20 border border-purple-500/50 text-purple-400">Multiplayer PK ⚡</span>
              <span class="px-2 py-1 text-xs font-semibold rounded bg-black/20 border border-emerald-500/50 text-emerald-400">Ready</span>
            </div>
          </div>
        </a>
        ...
        <div class="relative overflow-hidden rounded-3xl border opacity-60 transition-all duration-300 cursor-not-allowed grayscale"
          ...
          <div class="p-6">
            ...
            <div class="mt-4 flex items-center space-x-2">
              <span class="px-2 py-1 text-xs font-semibold rounded bg-black/20 border border-yellow-500/50 text-yellow-400">Development (Phase 4)</span>
            </div>
          </div>
        </div>
