...
func Seed() {
	games := []domain.GameConfig{
		...
		},
		{
			...
			Rules:    `{"en": "# Block Puzzle Rules\n\nDrag and drop blocks onto the grid. Fill an entire row or column to clear it.\n\n## 🎮 Controls\n- **Drag & Drop**: Move the shapes to the board.", "zh": "# 方块消除规则\n\n将下方给出的方块拖放到棋盘中，填满任意一行或一列即可消除得分。\n\n## 🎮 操作说明\n- **拖放**: 用鼠标或手指按住方块并拖入棋盘。"}`,
			Config:   `{}`,
			IsActive: true,
		},
	}
