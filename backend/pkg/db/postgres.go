...
func Seed() {
	...
	DB.Model(&domain.GameConfig{}).Where("id = ?", "sudoku").Update("rules", defaultSudoku.Rules)
	DB.Model(&domain.GameConfig{}).Where("id = ?", "sliding").Update("rules", defaultSliding.Rules)
	DB.Model(&domain.GameConfig{}).Where("id = ?", "hexa").Update("rules", defaultHexa.Rules)
	DB.Model(&domain.GameConfig{}).Where("id = ?", "tetris").Update("rules", defaultTetris.Rules)
